const { Integer, Boolean, Null, ReturnValue, Error } = require('../lib/object.js');
...
class Interpreter {
  ...
  eval(node) {
    switch(node.constructor.name) {
      ...
      case ('BlockStatement'): {
        return this.evalBlockStatment(node.statements)
      }
    }
  }
  ...
  evalProgram(node) {
    ...
    for (const stmt of node.statements) {
      ...
      if (result.type() == 'RESULT_VALUE' && result.value) {
        return result.value;
      }
    }
    return result;
